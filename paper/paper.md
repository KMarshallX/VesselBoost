...
To qualitatively evaluate our software, we used 3D MRA image slabs with a diverse range of image resolution (from 400 $\mu$m to 150 $\mu$m [@mattern_prospective_2018]). \autoref{fig:2}a shows the maximum intensity projections (MIP) of the original input images, and \autoref{fig:2}b shows the predicted segmentation using *predict* (note that here we use the model trained on manually corrected labels from the SMILE-UHURA challenge). We found that even though our models were trained on lower resolution data, i.e., using 3D multi-slab time-of-flight MRA with an isotropic resolution of 300 $\mu$m, it can be generalized to MRA images with varying resolutions.
...
Moreover, \autoref{fig:3} demonstrates that *TTA* can offer an extra segmentation boost when pre-trained models are trained on imperfect training labels (here, we use the OMELLETE labels [@mattern_2021]). In \autoref{fig:3}a is shown the MIP of the original input images, with 'zoomed in' patches. \autoref{fig:3}b shows the initial segmentation of an OMELETTE-based pre-trained model. Note how this pre-trained model cannot segment the smallest vessel shown in the 'zoomed in' patches. Despite being imperfect, these segmentations can be leveraged as proxy segmentations for *TTA*. Accordingly, \autoref{fig:3}c shows the boosted segmentations after using *TTA*. It is possible to observe that *TTA* improves the segmentation of the smallest vessels (see 400 $\mu$m and 300 $\mu$m images) and segmentation continuity (see 160 $\mu$m and 150 $\mu$m images).

![Qualitative evaluation of *Module 2* (*TTA*) from **VesselBoost**. (a) Maximum intensity projection of input images with a diverse range of image resolutions. (b) Predicted segmentations of an OMELETTE-based pre-trained model. (c) *TTA* based segmentation using the segmentation shown in (b) as proxy segmentation to guide model adaptation. The 'zoomed in' patches show the missing small vessels in panel (b) and are recovered with *TTA*. \label{fig:3}](figure3.png)

Finally, \autoref{fig:4} shows the utility of *boost* to train a segmentation model from scratch using imperfect training labels (middle) from a new data set. By leveraging the self-similarity of large and small vessels through data augmentation, it is possible to train a model from scratch (on a single or more data) using imperfect labels, such that it will have the capacity to segment the smallest vessel, and hence, improve the segmentation results beyond the training data. Importantly, this occurs so long as the voxel intensity of the smallest vessels is sufficiently similar to the ones from the largest ones.
