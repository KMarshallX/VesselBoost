...
To quantitatively evaluate the test-time adaptation module, we chose 14 subjects from StudyForrest project [@forstmann_multi-modal_2014], these 7T angiography images were with an isotropic resolution of 300 $\mu m$. 13 images were used for pre-training a model with a set of suboptimal segmentations (O1) for 5000 epochs, the pre-trained model was then used for TTA (100 epochs) test on one image slab. A manually annotated segmentation of the test image slab was used as a baseline to calculate the Dice score. We compared the Dice scores of the segmentation generated by the pre-trained model and the segmentation after test-time adaptation. As shown in \autoref{fig:3}, the Dice score increased from 0.5726 to 0.6371 after test-time adaptation. This result demonstrates that our test-time adaptation module can improve the segmentation accuracy of our pre-trained model.
