...
To quantitatively evaluate the TTA module, we trained a model using 13 MRA images from the SMILE-UHURA challenge, leaving one sample out for evaluation. Specifically, we used the set of conservative segmentations (OMELETTE 1) for model training for 5000 epochs. Then, this pre-trained model was used for proxy generation and adapted for 100 epochs using our TTA module and the test image. To remove unwanted false-positive voxels from outside the brain, a semi-automatically derived brain mask was applied. \autoref{fig:3} shows the conservative segmentation (O1), the ground-truth (or manually annotated) segmentation, the initial segmentation (proxy), and the final segmentation after TTA for the test image. Using the manually annotated segmentation as reference, the final segmentation after TTA showed in increase in Dice score of 0.06 compared to the initial proxy segmentation.
...
An early prototype of this work was presented at the 12th Scientific Symposium on Clinical Needs, Research Promises and Technical Solutions in Ultrahigh Field Magnetic Resonance in Berlin in 2021. This work was also submitted to the SMILE-UHURA challenge (https://www.soumick.com/en/uhura/).
