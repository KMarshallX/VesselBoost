...
![*VesselBoost* overview. (a) The test-time adaptation module (or Module 1) includes image pre-processing (i), model adaptation (ii), and post-processing (iii). (b) If no proxy is provided to the TTA module, a proxy segmentation is generated by the inference module (Module 2), which includes image pre-processing (i), inference using our pre-trained models (ii), and post-processing (iii).\label{fig:1}](figure1_v1.png)
...
To qualitatively evaluate our software, we used a 3D MRA image slab with an isotropic resolution of 150 $\mu m$ [@mattern_prospective_2018]. \autoref{fig:2}a shows the maximum intensity projections (MIP) of the original input image, the proxy segmentation generated by our pre-trained model, and the final segmentation after test-time adaptation. In \autoref{fig:2}b, we illustrate 'zoomed in' patches of the images shown in panel **a**. These patches showcase *VesselBoost*'s robustness in improving the continuity of the proxy segmentation by leveraging TTA to boost upon this coarser segmentation. Overall, we found that even though our models were trained on lower resolution data, it can be generalized to images with varying resolutions.

![Qualitative evaluation of *VesselBoost* on 3D MRA image slab with an isotropic resolution of 150 um. (a) Maximum intensity projection of the input image, the proxy segmentation generated by our pre-trained model, and the segmetation after test-time-adaptation. (b) 'Zoomed in' patches demonstrate a boost in segmentation details. \label{fig:2}](figure2_v11.png)

## Quantitative evaluation

To quantitatively evaluate the test-time adaptation module, we trained a model using 13 MRA images from the SMILE-UHURA challenge, leaving one sample out for evaluation. Specifically, we used the set of suboptimal segmentations (OMELETTE 1) for model training for 5000 epochs. Then, this pre-trained model was used for proxy generation and adapted for 100 epochs using our TTA module and the test image. \autoref{fig:3} shows the suboptimal segmentation (O1), the ground-truth (or manually annotated) segmentation, the initial segmentation (proxy), and the final segmentation after TTA for the test image. Note that, compared to manually annotated segmentation, the initial segmentation has a lower Dice score than the final segmentation. Specifically, we found a boost in the Dice score, i.e., an increase of 0.06 in the Dice score after test-time adaptation. This result demonstrates that our test-time adaptation module can improve the segmentation results beyond the training data.

![Quantitative evaluation of *VesselBoost* on 3D MRA image slab with an isotropic resolution of 300 um. Dice scores were estimated for the initial segmentation and the final segmentation, with the ground truth image as reference. In green we show the boost in Dice score after test-time adaptation. \label{fig:3}](figure4_v5.png)
