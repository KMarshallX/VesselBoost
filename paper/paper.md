...
![Qualitative evaluation of *VesselBoost* on 3D MRA image slab with an isotropic resolution of 150 $\mu m$. (a) Maximum intensity projection of the input image, the proxy segmentation generated by our pre-trained model, and the segmetation after test-time-adaptation. (b) 'Zoomed in' patches demonstrate a boost in segmentation details. \label{fig:2}](figure2_v11.png)

## Quantitative evaluation

To quantitatively evaluate the test-time adaptation module, we further trained new models using 13 images from the SMILE-UHU  chose 14 subjects from StudyForrest project [@forstmann_multi-modal_2014], these 7T angiography images were with an isotropic resolution of 300 $\mu m$. 13 images were used for pre-training a model with a set of suboptimal segmentations (O1) for 5000 epochs, and then fintuning the model (100 epochs) on one image slab. We compared the Dice score of the segmentation generated by the pre-trained model and the segmentation after test-time adaptation. As shown in \autoref{fig:3}, the Dice score increased from 0.5726 to 0.6371 after test-time adaptation. This result demonstrates that our test-time adaptation module can improve the segmentation accuracy of our pre-trained model.

![Quantitative evaluation on 300 um image data. A model pre-trained on suboptimal segmentations used for test-time adaptation, the result showed an increase in Dice score.  \label{fig:3}](figure4_v5.png)
